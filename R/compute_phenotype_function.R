...
calcPhenotype <- function(trainingExprData, trainingPtype, testExprData, batchCorrect="eb", powerTransformPhenotype=TRUE, removeLowVaryingGenes=.2, minNumSamples=10, selection=-1, printOutput=TRUE, removeLowVaringGenesFrom="homogenizeData")
{
  ...
  if(!c("matrix") %in% class(testExprData)) stop("ERROR: \"testExprData\" must be a matrix.");
  if(!c("matrix") %in% class(trainingExprData)) stop("ERROR: \"trainingExprData\" must be a matrix.");
  if(!c("numeric") %in% class(trainingPtype)) stop("ERROR: \"trainingPtype\" must be a numeric vector.");
  ...
  if(c("numeric") %in% class(homData$test))
  {
